...
PythonOutput PythonExporter::generate(const Path& path, double stepCm, Format fmt) {
    if (path.isEmpty())
    ...
    if (fmt == Format::Inline) {
        // ── Inline: var = [(x, y), ...] on a single line ────────────────────
        wp = var + " = [";
        for (int i = 0; i < n; ++i) {
            if (i) wp += ", ";
            wp += QString("(%1, %2)").arg(wps[i].x, 0, 'f', 2).arg(wps[i].y, 0, 'f', 2);
        }
        wp += "]";
